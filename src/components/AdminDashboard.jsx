...
import { Home, BarChart as BarChartIcon, CreditCard, Users, Wallet,Package ,LayoutDashboard} from 'lucide-react';
...
const AdminDashboard = () => {
  ...
  console.log('====================================');
  const orderUpdates = [
    { icon: '🛒', label: 'Average Orders', value:3000 },
    { icon: '🚚', label: 'Average Shipments', value: 2200 },
    { icon: '📦', label: 'Return to Origin', value: 80 }
  ];

  const performanceDetails = [
    { label: 'Calls per Hour Received', value: '22/25', color: 'yellow' },
    { label: 'Refund Initiated', value: '5/25', color: 'green' },
    { label: 'Discount Initiated', value: '7/25', color: 'orange' }
  ];
  ...
  const menuItems = [
    { path: "/adminDashboard", icon: <LayoutDashboard className={`w-5 h-5 ${location?.pathname === '/adminDashboard' ? 'text-green-700 ':''}`} />, label: "Dashboard" },
    { path: "/users", icon: <FaRegUser className={`w-5 h-5`} />, label: "Users" },
    { path: "/sellers", icon: <AiTwotoneShop className={`w-5 h-5`} />, label: "Sellers" },
    ...
    <div className="flex">
      ...
      <div className="w-64 bg-white border-r h-screen p-4">
        <div className="flex items-center mb-8">
          <img src={logo} alt="Flipkart Ads Logo" className="mr-2 h-[70px] w-[70px]" />
        </div>
        <nav>
          {menuItems.map((item, index) => (
            <div
              key={index}
              className={`mt-2 flex items-center p-2 hover:bg-gray-100 rounded cursor-pointer ${location?.pathname === '/adminDashboard' && item?.label === 'Dashboard' ? 'text-green-800 bg-green-100 hover:bg-green-100':''}`}
